...
def test_route_on_network(client):
    nid = client.post("/v1/networks", json={"kind": "grid", "rows": 4, "cols": 4}).json()["network_id"]
...
def test_solve_on_network(client):
    nid = client.post("/v1/networks", json={"kind": "grid", "rows": 6, "cols": 6, "spacing_m": 100}).json()["network_id"]
    body = {
        "network_id": nid,
        "depot_node": 0,
        "stops": [
...
def test_solve_validation_errors(client):
    ...
    assert client.post("/v1/route", json={"network_id": "nope", "origin": 0, "destination": 1}).status_code == 404
