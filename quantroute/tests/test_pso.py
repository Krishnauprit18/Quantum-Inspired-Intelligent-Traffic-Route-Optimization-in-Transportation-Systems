...
def test_pso_is_deterministic_for_a_seed():
    ...
    a = PSOOptimizer(PSOConfig(swarm_size=16, max_iterations=60, seed=5)).solve(toy_problem(), termination=term)
    b = PSOOptimizer(PSOConfig(swarm_size=16, max_iterations=60, seed=5)).solve(toy_problem(), termination=term)
...
def test_pso_warm_start():
    res = PSOOptimizer(PSOConfig(swarm_size=10, max_iterations=3, seed=0, warm_start_fraction=0.5)).solve(
        toy_problem(), warm_start=np.array([0.1, 0.2, 0.3, 0.4]),
