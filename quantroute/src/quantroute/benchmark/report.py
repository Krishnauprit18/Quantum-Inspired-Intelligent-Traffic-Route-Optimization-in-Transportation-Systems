...
def write_report(
    ...
    *,
    ...
    for r in records:
        cdf = pd.DataFrame(list(r.convergence), columns=["iteration", "elapsed_s", "incumbent_cost"])
        cdf.to_csv(
...
def _maybe_plot(records: list[RunRecord], out: Path) -> None:
    ...
    for instance, recs in by_instance.items():
        ...
        for algo in algos:
            traces = [
                pd.DataFrame(list(r.convergence), columns=["iteration", "elapsed_s", "incumbent_cost"])
                for r in recs
            ...
            merged = (
                pd.concat(traces)
                .groupby("iteration", as_index=False)["incumbent_cost"]
                .mean()
            )
