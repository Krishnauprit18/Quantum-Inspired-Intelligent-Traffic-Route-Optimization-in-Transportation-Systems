...
def _to_record(inst, inst_path: str, algo_id: str, seed: int, result, report_target_gap: float) -> RunRecord:
    bk = inst.best_known
    gap_pct = (
        (result.best_cost - bk) / bk * 100.0 if (bk is not None and bk > 0) else None
    )
