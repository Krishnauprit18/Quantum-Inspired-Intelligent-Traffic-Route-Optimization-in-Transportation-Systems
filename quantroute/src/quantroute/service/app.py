...
def create_app() -> FastAPI:
    ...
    app = FastAPI(title="quantroute", version=__version__, description=__doc__, lifespan=lifespan)
...
def _build_graph(req):
    if isinstance(req, GridNetworkRequest):
        return grid_city(req.rows, req.cols, spacing_m=req.spacing_m, arterial_every=req.arterial_every)
...
def _solve_to_dict(req: SolveRequest, networks: NetworkStore) -> dict:
    ...
    return {
        ...
        "routes": [
            {"vehicle": r.vehicle_id, "stops": list(r.stop_ids)} for r in result.routes if r.stop_ids
        ],
...
def _assemble(req: SolveRequest, networks: NetworkStore):
    ...
    if req.network_id is not None:
        ...
        coords = {i: (float(x), float(y)) for i, x, y in zip(ids, xs, ys)}
        graph = epoch = None
