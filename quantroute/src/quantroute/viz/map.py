...
def routes_to_geojson(
    ...
    *,
    ...
    features.append(
        _point(coords[depot_node], {"role": "depot", "node": int(depot_node)}, "depot")
    )

    for ri, route in enumerate(r for r in routes if r.stop_ids):
        color = ROUTE_COLORS[ri % len(ROUTE_COLORS)]
        seq_nodes = [int(depot_node), *(int(stop_node[sid]) for sid in route.stop_ids), int(depot_node)]
        ...
        for sid in route.stop_ids:
            ...
            features.append(
                _point(coords[node], {"role": "stop", "id": sid, "vehicle": route.vehicle_id, "color": color}, "stop")
            )
...
def render_map_html(feature_collection: dict, *, title: str = "quantroute", geographic: bool | None = None) -> str:
...
def render_map_png(feature_collection: dict, out, *, title: str = "quantroute"):
    ...
    for feat in feature_collection["features"]:
        ...
        if geom["type"] == "LineString":
            ...
            x, y = geom["coordinates"]
            ax.plot(x, y, "o", color=props.get("color", "#888"), markersize=6, mec="black", mew=0.5, zorder=4)
