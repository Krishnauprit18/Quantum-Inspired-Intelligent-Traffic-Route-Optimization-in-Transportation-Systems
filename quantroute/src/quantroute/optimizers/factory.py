...
class OptimizerFactory:
    ...
    @classmethod
    def register(
        cls, name: str, optimizer_cls: type[Optimizer], config_cls: type
    ) -> None:
    ...
    def from_config(cls, config: Mapping[str, Any]) -> Optimizer:
        ...
        if key not in cls._registry:
            raise KeyError(
                f"unknown algorithm {name!r}; available: {', '.join(cls.available())}"
            )
        ...
        if unknown:
            raise ValueError(
                f"unknown params for {key!r}: {sorted(unknown)}; "
