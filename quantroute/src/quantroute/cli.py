...
def solve(
    ...
    objective: str = typer.Option("travel-time", "--objective", help="travel-time | distance | makespan"),
    no_repair: bool = typer.Option(False, "--no-repair", help="disable the capacity repair operator"),
    ...
    map_html: Path = typer.Option(None, "--map-html", help="draw the routes to a standalone HTML map"),
...
def _make_optimizer(algorithm: str, seed: int, iterations: int, pop_size: int | None, config: Path | None):
    ...
    try:
        with path.open("w", encoding="utf-8", newline="") as fh:
            ...
            for rec in result.convergence:
                writer.writerow([rec.iteration, f"{rec.elapsed_s:.6f}", f"{rec.incumbent_cost:.6f}"])
...
def benchmark(
    ...
    output_dir: Path = typer.Option(None, "--output-dir", "-o", help="override the plan's output_dir"),
...
def demo(
    ...
    map_html: Path = typer.Option(None, "--map-html", help="draw the free-flow routes to an HTML map"),
) -> None:
    ...
    def _solve(epoch, label: str):
        ...
        opt = OptimizerFactory.from_config(
            {"algorithm": algorithm, "params": {"seed": seed, "max_iterations": iterations, key: 30}}
        )
    ...
    typer.echo(f"grid {rows}x{cols}: {n_nodes} nodes, {graph.num_edges} edges | depot {depot} | {stops} stops")
    ...
    if map_html is not None:
        ...
        fc = routes_to_geojson(
            free_res.routes, coords, depot, stop_node=stop_node,
            road_graph=graph, epoch=free_epoch,
        )
...
def serve(
    ...
    uvicorn.run(
        "quantroute.service:create_app", host=host, port=port, factory=True, reload=reload
    )
