...
def test_moves_stops_until_capacity_is_respected():
    spec, matrix = build(
        demands=[8, 8, 8, 8], caps=[20, 20], coords=[(1, 0), (2, 0), (3, 0), (4, 0)]
    )
...
def test_repair_is_total_on_genuinely_infeasible_input():
    ...
    spec, matrix = build(
        demands=[15, 15, 15, 15], caps=[20, 20], coords=[(1, 0), (2, 0), (3, 0), (4, 0)]
    )
    broken = Routes.from_lists({"v1": ["c0", "c1", "c2", "c3"], "v2": []})
...
def test_repair_leaves_feasible_solution_untouched():
    spec, matrix = build(
        demands=[10, 10, 10, 10], caps=[30, 30], coords=[(10, 0), (20, 0), (0, 10), (0, 20)]
    )
