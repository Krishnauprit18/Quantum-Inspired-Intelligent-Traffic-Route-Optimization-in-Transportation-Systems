...
def test_ga_is_deterministic_and_monotone():
    ...
    a = GAOptimizer(GAConfig(population_size=20, max_iterations=60, seed=7)).solve(
        toy_problem(), termination=term
    )
    b = GAOptimizer(GAConfig(population_size=20, max_iterations=60, seed=7)).solve(
        toy_problem(), termination=term
    )
...
def test_aco_is_deterministic_and_monotone():
    term = TerminationCriteria(max_iterations=60)
    a = ACOOptimizer(ACOConfig(n_ants=16, max_iterations=60, seed=5)).solve(
        toy_problem(), termination=term
    )
    b = ACOOptimizer(ACOConfig(n_ants=16, max_iterations=60, seed=5)).solve(
        toy_problem(), termination=term
    )
    assert np.array_equal(a.best_vector, b.best_vector)
    costs = [r.incumbent_cost for r in a.convergence]
