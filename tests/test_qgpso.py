...
def test_qgpso_positions_stay_in_unit_interval():
    p = toy_problem()
    opt = QGPSOOptimizer(
        QGPSOConfig(population_size=16, max_iterations=40, seed=2, mutation_rate=0.1)
    )
...
def test_qgpso_deterministic_for_seed():
    ...
    a = QGPSOOptimizer(QGPSOConfig(population_size=16, max_iterations=50, seed=9)).solve(
        toy_problem(), termination=term
    )
    b = QGPSOOptimizer(QGPSOConfig(population_size=16, max_iterations=50, seed=9)).solve(
        toy_problem(), termination=term
    )
...
def test_qgpso_anytime_incumbent_monotone():
    ...
    QGPSOOptimizer(
        QGPSOConfig(population_size=12, max_iterations=60, seed=3, publish_every=10)
    ).solve(toy_problem(), callback=cb, termination=TerminationCriteria(max_iterations=60))
...
def test_qgpso_rejects_non_unit_encoding():
    ...
    with pytest.raises(NotImplementedError, match=r"\[0, 1\] random-key"):
        QGPSOOptimizer(QGPSOConfig(max_iterations=2)).solve(
            bad, termination=TerminationCriteria(max_iterations=2)
        )
