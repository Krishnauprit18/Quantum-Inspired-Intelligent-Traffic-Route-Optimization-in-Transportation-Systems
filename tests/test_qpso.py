...
def toy_problem(**ev_kw) -> SearchProblem:
    ...
    ev = FitnessEvaluator(
        inst.spec, inst.matrix, enc, repair=GreedyRepair(inst.spec, inst.matrix), **ev_kw
    )
...
def test_run_is_deterministic_for_a_seed():
    ...
    a = QPSOOptimizer(QPSOConfig(swarm_size=16, max_iterations=60, seed=7)).solve(
        toy_problem(), termination=term
    )
    b = QPSOOptimizer(QPSOConfig(swarm_size=16, max_iterations=60, seed=7)).solve(
        toy_problem(), termination=term
    )
...
def test_stagnation_stops_the_run():
    res = QPSOOptimizer(QPSOConfig(swarm_size=24, max_iterations=5000, seed=2)).solve(
        ...
        SearchProblem(
            toy_problem().spec,
            toy_problem().matrix,
            toy_problem().encoding,
            toy_problem().evaluator,
        ),
...
def test_warm_start_seeds_the_swarm():
    ...
    res = QPSOOptimizer(
        QPSOConfig(swarm_size=10, max_iterations=3, seed=0, warm_start_fraction=0.5)
    ).solve(p, warm_start=good, termination=TerminationCriteria(max_iterations=3))
...
def test_island_exchange_hook_can_inject_a_better_gbest():
    ...
    res = QPSOOptimizer(
        QPSOConfig(swarm_size=12, max_iterations=20, seed=0, exchange_every=5)
    ).solve(p, termination=TerminationCriteria(max_iterations=20), on_exchange=exchange)
