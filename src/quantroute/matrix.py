...
class DenseCostMatrix:
    ...
    def _validate_square(arr: np.ndarray, n: int, label: str) -> np.ndarray:
        ...
        if a.shape[0] != n:
            raise ValueError(f"{label} shape {a.shape} does not match node count {n}")
...
def euclidean_matrix_2d(
    ...
    *,
    ...
    if x.shape != (n,) or y.shape != (n,):
        raise ValueError(
            f"coordinate arrays must each have length {n}, got xs={x.shape}, ys={y.shape}"
        )
    if n > max_nodes:
        raise ValueError(f"euclidean_matrix_2d: {n} nodes exceeds max_nodes={max_nodes}")
