...
class ORToolsConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.first_solution not in _FIRST_SOLUTION_STRATEGIES:
            raise ValueError(f"first_solution must be one of {sorted(_FIRST_SOLUTION_STRATEGIES)}")
        ...
        **_ignored,
        ...
        def _on_solution() -> None:
            ...
            if current < best_so_far:
                best_so_far = current
                improvements.append((len(improvements) + 1, time.perf_counter() - started, current))
