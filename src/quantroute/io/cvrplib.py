...
def _to_int(token: str, lineno: int, what: str) -> int:
    try:
        ...
        raise CVRPLIBParseError(
            f"line {lineno}: expected integer for {what}, got {token!r}"
        ) from None
...
def _to_float(token: str, lineno: int, what: str) -> float:
    try:
        ...
        raise CVRPLIBParseError(
            f"line {lineno}: expected number for {what}, got {token!r}"
        ) from None
...
def _to_finite_float(token: str, lineno: int, what: str) -> float:
    ...
    if not math.isfinite(value):
        raise CVRPLIBParseError(f"line {lineno}: {what} must be a finite number, got {token!r}")
    return value


def _extract_best_known(comment: str) -> float | None:
    m = re.search(
        r"(?:optimal|best)\s+value\s*[:=]?\s*([0-9]+(?:\.[0-9]+)?)", comment, re.IGNORECASE
    )
    ...
    *,
    ...
    for lineno, raw in enumerate(lines, start=1):
        ...
        if section == "NODE_COORD_SECTION":
            ...
            raise CVRPLIBParseError(f"line {lineno}: section {section} is not supported yet")
    ...
    stops = tuple(
        Stop(id=f"c{n}", node=n, demand=demands[n]) for n in ordered_nodes if n != depot_node
    )
    vehicles = tuple(
        Vehicle(id=f"v{i + 1}", capacity=capacity, start_depot="depot") for i in range(resolved)
    )
    spec = ProblemSpec(
        name=name,
        depots=(depot,),
    ...
    *,
    ...
    if header_value is not None:
        try:
            ...
            raise CVRPLIBParseError(
                f"VEHICLES header is not an integer: {header_value!r}"
            ) from None
        if v < lower_bound:
            raise CVRPLIBParseError(f"VEHICLES={v} below capacity lower bound {lower_bound}")
